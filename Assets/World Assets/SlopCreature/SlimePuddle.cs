...
public class SlimePuddle : MonoBehaviour
{
    ...
    [Tooltip("Only colliders with this tag take damage. Prevents the slime damaging itself or allied NPCs.")]
    ...
    IEnumerator TickLoop()
    {
        ...
        while (Time.time < end)
        {
            ...
            foreach (var col in snap)
            {
                ...
                // Only damage the correct victim tag (default: Player)
                bool isVictim = string.IsNullOrEmpty(victimTag) || col.CompareTag(victimTag)
                    || (col.GetComponentInParent<Transform>() is Transform pt && pt.CompareTag(victimTag));
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        // Only apply slow/damage to the victim tag — ignore slime allies and NPCs
        bool isVictim = string.IsNullOrEmpty(victimTag) || other.CompareTag(victimTag)
            || (other.GetComponentInParent<Transform>() is Transform pt && pt.CompareTag(victimTag));
        if (!isVictim) return;
